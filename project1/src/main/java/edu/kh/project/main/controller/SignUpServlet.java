...
//		req.setCharacterEncoding("UTF-8");
		...
		//주소는 선택적으로 입력하는 값 -> 미입력시 {,,} (모든 인덱스가 빈칸)
		String [] arr = req.getParameterValues("memberAddress"); //3개(name값 3개)
		...
		if(!arr[0].equals("") && !arr[1].equals("") && !arr[2].equals("")) {
			...
			//String.join("구분자", 배열) :
			// - 모든 배열 요소를 하나의 문자열로 변환
			// - 요소 사이사이에 "구분자" 추가
			
		}
		//Member 객체 생성 후 파라미터 세팅하기
		...
		try {
			...
			// insert 진행 -> 반영된 행의 개수 반환 -> int 자료형
			int result = service.signUp(member);
			
			// 서비스 수행 결과에 따라 결과화면 제어
			String path = null;
