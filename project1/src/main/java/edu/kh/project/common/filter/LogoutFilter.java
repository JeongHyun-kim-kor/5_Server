...
public class LogoutFilter extends HttpFilter implements Filter {
	...
	public void init(FilterConfig fConfig) throws ServletException {
		System.out.println("로그아웃상태인지 검사하는 필터 초기화");
	}


	
	public void destroy() {
		System.out.println("로그아웃상태인지 파괴하는 필터 초기화");
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// Session 객체는 HttpServletRequest(자식)에서만 얻어올 수 있다.
		// -> 다운 캐스팅 필요
		
		HttpServletRequest req = (HttpServletRequest)request;
		HttpServletResponse resp = (HttpServletResponse)response; // 리 다이렉트용
