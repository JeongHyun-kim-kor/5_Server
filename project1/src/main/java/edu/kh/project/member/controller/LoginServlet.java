...
//			req.setAttribute("loginMember", loginMember);
			...
			// 1) HttpSesscion 객체 얻어오기
			// 요청자의 session정보를 얻어오는거기 때문
...
//			------------------------------------------------------------
			...
			Cookie cookie = new Cookie("saveId", inputEmail);  // key value 형태
			
			// 1) 아이디 저장 체크박스가 체크 되었는지 확인
			if(req.getParameter("saveId") != null) { // 체크된 경우
			
//				3) 쿠키가 유지될 수 있는 유효기간 설정(초 단위)
				cookie.setMaxAge(60*60*24*30); //30일 3600*24*30
				
				
			} else { //체크되지 않은 경우
				
				// 4) 쿠키의 유효기간을 0초로 설정
				// == 클라이언트에 저장된 saveId쿠키를 삭제하라는 의미
				// (같은 key값의 쿠키가 저장되면 덮어쓰기가 됨)
				cookie.setMaxAge(0); 

			}
			// 5) 생성된 쿠기가 적용되어질 요청 주소를 지정
			...
			// path가 "/" 인 경우 main페이지로 redirect한다.

			// 메인 페이지로 redirect
			// -> 메인 페이지를 요청하는 것이기 때문에
