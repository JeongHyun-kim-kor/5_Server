...
public class ServletEx2 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// select 박스는 1개의 옵션만 선택 가능 == 값 1개만 서버로 제출됨
		String coffee = req.getParameter("coffee");
		
		// checkbox와 같이 하나의 name 속성으로 여러 값이 전달 될 경우
		
		// req.getParameter()       -> 여러 값 중 첫 번째 값만 얻어옴 (String 타입)
		// req.getParameterValues() -> 여러 값을 모두 얻어옴          (String[] 타입 )
//									단, 값이 하나도 없으면 null
...
//		String opt = req.getParameter("opt");
		...
		// (HTML, CSS, JS 등 브라우저가 해석할 ㅅ 있는 코드는 모드 해석되서 보여짐)
